...
%START_LATEX
\begin{enumerate} 
\item PAC D5005: add -DUSE_SVM=Yes to build the host code with SVM support.
\item U280: modify the link settings file path to match the link configuration for HBM or DDR. Disable the link setting generation in case of DDR.
\end{enumerate}
%STOP_LATEX
...
We executed the benchmark with the stream_kernels_multi.xclbin and arrays of size 2^28, since 2^29 does not fit into the used memory.
